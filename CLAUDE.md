...
- `ODOO_WORKSPACE` is a fixed external directory containing `CLAUDE.md`,
  `VERSIONS.md`, and `versions/<version>/{odoo,documentation}/`. The legacy
  single-version configuration remains supported. Do not copy Odoo into this repo.
...
- Without a version in the current request, use the numerically newest installed
  ...
  alone does not count as installed code. An explicit version must be respected.
